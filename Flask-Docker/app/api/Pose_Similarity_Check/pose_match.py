...
from Pose_Similarity_Check.openpose_skeleton.skeleton import draw
import pandas as pd
...
def get_pose_similarity(video_path1, video_path2, exercise):# main function
    ...
    if len(similarity_list) > 0:
        ...
        num_top_pairs = min(5, len(similarity_array)) # 결과 pair 수(현재 5)
        ...
        #     #sk_frame1.append(draw(frame1, index))
        #     #sk_frame2.append(draw(frame2, index))
        #     #cv2.imwrite(f"poseResult/test1_{i+1}.png", sk_frame1[i]) #test (pro)
        #     #cv2.imwrite(f"poseResult/test2_{i+1}.png", sk_frame2[i]) #test (user)
    ...
    plot_similarity(similarity_list, 'tempDB/image/graph.png')
    return similarities, frame_pairs, top_indices